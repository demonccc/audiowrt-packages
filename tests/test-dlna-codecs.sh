...
for package in lpcm m4a aiff opus; do
    ...
    test -f "audiowrt-player-$package/files/$package.manifest" || fail "$package registration missing"
    grep -q "+libaudiowrt-player" "audiowrt-player-$package/Makefile" ||
        fail "$package does not use shared player library"
done

grep -q 'audio/L16' audiowrt-player-lpcm/files/lpcm.manifest || fail "LPCM MIME missing"
...
grep -q 'audio/aiff' audiowrt-player-aiff/files/aiff.manifest || fail "AIFF MIME missing"
...
grep -q 'alac ffmpeg_audio' audiowrt-player-ffmpeg/files/ffmpeg.manifest ||
    fail "ALAC registration missing"
grep -q 'wma ffmpeg_audio' audiowrt-player-ffmpeg/files/ffmpeg.manifest ||
    fail "WMA registration missing"
grep -q 'm4a ffmpeg_audio' audiowrt-player-ffmpeg/files/ffmpeg.manifest ||
    fail "M4A FFmpeg fallback missing"

for defaults in     audiowrt-player-lpcm/files/lpcm.manifest     audiowrt-player-m4a/files/m4a.manifest     audiowrt-player-aiff/files/aiff.manifest     audiowrt-player-opus/files/opus.manifest     audiowrt-player-ffmpeg/files/ffmpeg.manifest; do
    sh -n "$defaults"
done
