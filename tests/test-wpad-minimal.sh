...
# Keep the multicall TLS/link setup aligned with OpenWrt's canonical recipe.
...
# The AP side is open-only. TLS backend selection is supplied by the canonical
# OpenWrt-style DRIVER_MAKEOPTS so both halves of the multicall share it.
for keep in \
